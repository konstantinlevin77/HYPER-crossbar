...
class HypergraphLayer(nn.Module):
    ...
    def forward(self, node_features, edge_list, rel, init_feature, relation_representations=None):
        ...
        if self.use_triton:
            ...
            cache_key = (
                edge_list_trans.data_ptr(),
                rel.data_ptr(),
                tuple(edge_list_trans.shape),
                tuple(rel.shape),
                node_size,
            )
            if self._triton_graph_cache is None or self._triton_graph_cache[0] != cache_key:
                self._triton_graph_cache = (
                    cache_key,
                    preprocess_triton_hypergraph(edge_list_trans, rel, num_node=node_size)[:4],
                )
            rowptr, indices, etypes, pos_index = self._triton_graph_cache[1]
