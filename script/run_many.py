...
default_finetuning_config = {
    ...
    # Hypergraph dataset transductive (8)
    "FBAUTO": (1, 'null'),
    "JF17K": (3, 'null'),
    "Wikipeople": (3, 'null'),
    "MFB15K": (3, 'null'),
    # Hypergraph dataset inductive (3)
    "JFIND": (1, 'null'),
    "WPIND": (1, 'null'),
    "MFBIND": (1, 2000),
...
default_train_config = {
    ...
    # Hypergraph dataset transductive (3)
    "FBAUTO": (20, 'null'),
    "JF17K": (10, 'null'),
    "Wikipeople": (10, 'null'),
    "MFB15K": (10, 'null'),
...
if __name__ == "__main__":
    ...
    results_file = os.path.join(path, f"hyper_results_{time.strftime('%Y-%m-%d-%H-%M-%S')}.csv")
    ...
    for graph in datasets:
        ...
        for i in range(args.repeats):
            ...
            if util.get_rank() == 0:
                neptune_logger = util.create_neptune_run(args, vars, cfg=cfg)
            set_seed(seed)
            ...
            if cfg.model["class"] == "HYPER":
                ...
                if not isinstance(num_relations, int) and len(num_relations) > 1:
                    num_relations = max(num_relations)
            ...
            if task_name == "InductiveInference":
                ...
                filtered_data = Data(edge_index=dataset.target_edge_index, edge_type=dataset.target_edge_type, num_nodes=dataset[0].num_nodes)
            ...
            train_and_validate(cfg, model, train_data, valid_data, filtered_data=val_filtered_data, device=device, logger=logger, neptune_logger=neptune_logger)
            if util.get_rank() == 0:
